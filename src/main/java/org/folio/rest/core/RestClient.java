...
public class RestClient {
  ...
  public <T> Future<T> post(
    ...
    if (log.isDebugEnabled()) {
      log.debug(
        REQUEST_MESSAGE_LOG_DEBUG,
        HttpMethod.POST,
        JsonObject.mapFrom(entity).encodePrettily()
      );
    }
    var caseInsensitiveHeader = convertToCaseInsensitiveMap(
  ...
  public Future<Void> postEmptyBody(
    RequestEntry requestEntry,
    RequestContext requestContext
  ) {
  ...
  protected MultiMap convertToCaseInsensitiveMap(
  ...
  public <T> Future<Void> put(
    ...
    if (log.isDebugEnabled()) {
      log.debug(
        REQUEST_MESSAGE_LOG_DEBUG,
        HttpMethod.PUT,
        JsonObject.mapFrom(recordData).encodePrettily()
      );
    }
    var caseInsensitiveHeader = convertToCaseInsensitiveMap(
      requestContext.getHeaders()
  ...
  private <T> void handleGetMethodErrorResponse(
    Promise<T> promise,
    Throwable t,
    boolean skipError404
  ) {
  ...
  public <T> Future<T> get(
    ...
    log.info("Headers: {}", caseInsensitiveHeader);
    log.info("Endpoint: {}", absEndpoint);
    ...
    getVertxWebClient(requestContext.getContext())
      ...
      .map(res -> {
        ...
        log.info("RES contents:", res.bodyAsString());
        return res;
      })
      .map(HttpResponse::bodyAsJsonObject)
      .map(jsonObject -> {
