...
import static org.folio.invoices.utils.ErrorCodes.PENDING_PAYMENT_CREATION_ERROR;
...
public class PendingPaymentWorkflowService {
  ...
  public Future<List<InvoiceWorkflowDataHolder>> handlePendingPaymentsCreation(List<InvoiceWorkflowDataHolder> dataHolders,
    ...
    // Encumbrance and expenditure restriction checks are done in mod-finance-storage.
    // If we need to add a step saving data before this, we might have to add a validation before.
    ...
    return getOldEncumbrancesToRelease(holders, invoice, requestContext)
      ...
      .recover(t -> {
        ...
        return recoverToForwardHttpException(t, PENDING_PAYMENT_CREATION_ERROR);
      })
      .map(v -> dataHolders);
  }
  ...
  public Future<Void> handlePendingPaymentsUpdate(List<InvoiceWorkflowDataHolder> dataHolders, RequestContext requestContext) {
    ...
    return baseTransactionService.batchUpdate(transactionsToUpdate, requestContext)
      .recover(t -> {
        log.error("handlePendingPaymentsUpdate: Failed to update pending payments", t);
        return recoverToForwardHttpException(t, PENDING_PAYMENT_UPDATE_ERROR);
      });
  }
  ...
  private Future<Void> recoverToForwardHttpException(Throwable t, ErrorCodes code) {
    ...
    Error error = code.toError().withParameters(List.of(causeParam));
