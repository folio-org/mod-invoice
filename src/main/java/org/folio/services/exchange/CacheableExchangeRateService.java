...
public class CacheableExchangeRateService {
  ...
  public Future<ExchangeRate> getExchangeRate(String from, String to, Number customExchangeRate, String operationMode, RequestContext requestContext) {
    if (StringUtils.equals(from, to)) {
      return Future.succeededFuture(createDefaultExchangeRate(from, to, 1d, ExchangeRate.OperationMode.MULTIPLY.name()));
    }
  ...
  private ExchangeRate createDefaultExchangeRate(String from, String to, Number exchangeRateValue, String operationMode) {
    return new ExchangeRate().withFrom(from).withTo(to)
      .withExchangeRate(exchangeRateValue.doubleValue())
      .withOperationMode(ExchangeRate.OperationMode.valueOf(operationMode));
  }
